...
for i in range(len(data_train_label)):
    x_data_train_label.append([data_train_label[i][0], data_train_label[i][1]])


Train_Idx = list(np.random.choice(list(range(len(data_test_label))),
                                               size=int(0.85 * len(data_test_label)), replace=False))
for idx in Train_Idx:
    x_data_train_label.append([data_test_label[idx][0], data_test_label[idx][1]])

x_data_train_label = np.array(x_data_train_label).reshape((len(x_data_train_label), 2))
x_data_test_label = data_test_label

x_data_train_label_p3, x_data_test_label_p3 = [], []
for i in range(len(x_data_train_label)):
    if 'P3' in x_data_train_label[i][0]:
        x_data_train_label_p3.append([x_data_train_label[i][0], x_data_train_label[i][1]])
    else:
        continue
for i in range(len(x_data_test_label)):
    if 'P3' in x_data_test_label[i][0]:
        x_data_test_label_p3.append([x_data_test_label[i][0], x_data_test_label[i][1]])

x_data_train_label_clean = []
case_removal = ['HKU_0427', 'ID_0160', 'ID_417', 'ID_0856b', 'QMH_0001', 'QMH_0139', 'SZH_0075', 'SZH_0404',
                'HKU_0065', 'HKU_0069', 'HKU_0389', 'QMH_0143', 'ID_0254', 'SZH_0409', 'ID_0444', 'QMH_0126',
                'ID_0106']

for i in range(len(x_data_train_label_p3)):
    data_path = x_data_train_label_p3[i][0]
    for case_id in case_removal:
        if not(case_id in data_path) and \
                not([x_data_train_label_p3[i][0], x_data_train_label_p3[i][1]] in x_data_train_label_clean):
            x_data_train_label_clean.append([x_data_train_label_p3[i][0], x_data_train_label_p3[i][1]])
        else:
            continue


x_data_train_label_clean = np.array(x_data_train_label_clean).reshape((len(x_data_train_label_clean), 2))
x_data_test_label_p3 = np.array(x_data_test_label_p3).reshape((len(x_data_test_label_p3), 2))

print("Number of samples in training and validation are: %d and %d" % (len(x_data_train_label), len(x_data_test_label)))
batch_size = 2
